...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add aws-production -u admin -p "AWSProd@2024!SecureKey" --notes "Production AWS account" 2>nul
if %errorlevel% equ 0 (echo   ✓ aws-production added) else (echo   ✗ Failed to add aws-production)

echo [2/15] Adding aws-dev...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add aws-dev -u developer -p "DevAWS#123Secure" --notes "Development AWS account" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add github-personal -u myusername -p "GitHub@Personal2024!" --notes "Personal GitHub account" 2>nul
if %errorlevel% equ 0 (echo   ✓ github-personal added) else (echo   ✗ Failed to add github-personal)

echo [4/15] Adding github-work...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add github-work -u work.user -p "WorkGitHub#Secure99" --notes "Work GitHub account" 2>nul
if %errorlevel% equ 0 (echo   ✓ github-work added) else (echo   ✗ Failed to add github-work)

REM Database Credentials
echo [5/15] Adding postgres-main...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add postgres-main -u dbadmin -p "PostgresMain@2024Secure!" --notes "Main PostgreSQL database" 2>nul
if %errorlevel% equ 0 (echo   ✓ postgres-main added) else (echo   ✗ Failed to add postgres-main)

echo [6/15] Adding mysql-dev...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add mysql-dev -u devuser -p "MySQLDev#SecurePass123" --notes "Development MySQL instance" 2>nul
if %errorlevel% equ 0 (echo   ✓ mysql-dev added) else (echo   ✗ Failed to add mysql-dev)

REM API Keys
echo [7/15] Adding stripe-api...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add stripe-api -u sk_live_key -p "sk_live_XXXXXXXXXXXXXXXXXXXXXXXX" --notes "Stripe production API key" 2>nul
if %errorlevel% equ 0 (echo   ✓ stripe-api added) else (echo   ✗ Failed to add stripe-api)

echo [8/15] Adding openai-api...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add openai-api -u project-key -p "sk-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" --notes "OpenAI API key for project" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add gmail-main -u user@gmail.com -p "Gmail@SecurePass2024!" --notes "Main Gmail account" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add paypal-business -u business@example.com -p "PayPal@Business2024Secure" --notes "Business PayPal account" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add azure-storage -u azureuser -p "Azure@Storage2024Secure!" --notes "Azure storage account" 2>nul
if %errorlevel% equ 0 (echo   ✓ azure-storage added) else (echo   ✗ Failed to add azure-storage)

REM GitLab
echo [12/15] Adding gitlab-work...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add gitlab-work -u gitlab.user -p "GitLab@Work2024Secure" --notes "Work GitLab instance" 2>nul
if %errorlevel% equ 0 (echo   ✓ gitlab-work added) else (echo   ✗ Failed to add gitlab-work)

REM MongoDB
echo [13/15] Adding mongodb-cluster...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add mongodb-cluster -u mongoadmin -p "MongoDB@Cluster2024!" --notes "MongoDB production cluster" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add "special-chars-test" -u "user@domain.com" -p "P@$$w0rd!#^&*()_+-=[]{}|;:',.<>?/" --notes "Testing special characters in password" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add random-service -u randomuser -p "RandomService@2024Secure" --notes "Uncategorized test service" 2>nul
...
echo.
echo Test Services:
echo   - AWS (aws-production, aws-dev, azure-storage)
echo   - Databases (postgres-main, mysql-dev, mongodb-cluster)
echo   - Version Control (github-personal, github-work, gitlab-work)
echo   - APIs (stripe-api, openai-api)
echo   - Communication (gmail-main)
echo   - Payment (paypal-business)
echo   - Special Characters (special-chars-test)
echo   - Other (random-service)
echo.
