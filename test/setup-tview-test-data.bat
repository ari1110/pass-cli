...
echo.
(echo %TEST_PASSWORD% & echo %TEST_PASSWORD%) | %BINARY% --vault %TEST_VAULT% init
if %errorlevel% neq 0 (
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add aws-production -u admin -p "AWSProd@2024!SecureKey" -c "Cloud" --url "https://console.aws.amazon.com" --notes "Production AWS account" 2>nul
if %errorlevel% equ 0 (echo   ✓ aws-production added) else (echo   ✗ Failed to add aws-production)

echo [2/15] Adding aws-dev...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add aws-dev -u developer -p "DevAWS#123Secure" -c "Cloud" --url "https://console.aws.amazon.com" --notes "Development AWS account" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add github-personal -u myusername -p "GitHub@Personal2024!" -c "Version Control" --url "https://github.com" --notes "Personal GitHub account" 2>nul
if %errorlevel% equ 0 (echo   ✓ github-personal added) else (echo   ✗ Failed to add github-personal)

echo [4/15] Adding github-work...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add github-work -u work.user -p "WorkGitHub#Secure99" -c "Version Control" --url "https://github.com" --notes "Work GitHub account" 2>nul
if %errorlevel% equ 0 (echo   ✓ github-work added) else (echo   ✗ Failed to add github-work)

REM Database Credentials
echo [5/15] Adding postgres-main...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add postgres-main -u dbadmin -p "PostgresMain@2024Secure!" -c "Databases" --url "postgresql://localhost:5432" --notes "Main PostgreSQL database" 2>nul
if %errorlevel% equ 0 (echo   ✓ postgres-main added) else (echo   ✗ Failed to add postgres-main)

echo [6/15] Adding mysql-dev...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add mysql-dev -u devuser -p "MySQLDev#SecurePass123" -c "Databases" --url "mysql://localhost:3306" --notes "Development MySQL instance" 2>nul
if %errorlevel% equ 0 (echo   ✓ mysql-dev added) else (echo   ✗ Failed to add mysql-dev)

REM API Keys
echo [7/15] Adding stripe-api...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add stripe-api -u sk_live_key -p "sk_live_XXXXXXXXXXXXXXXXXXXXXXXX" -c "APIs" --url "https://stripe.com/docs/api" --notes "Stripe production API key" 2>nul
if %errorlevel% equ 0 (echo   ✓ stripe-api added) else (echo   ✗ Failed to add stripe-api)

echo [8/15] Adding openai-api...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add openai-api -u project-key -p "sk-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" -c "AI Services" --url "https://platform.openai.com" --notes "OpenAI API key for project" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add gmail-main -u user@gmail.com -p "Gmail@SecurePass2024!" -c "Communication" --url "https://mail.google.com" --notes "Main Gmail account" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add paypal-business -u business@example.com -p "PayPal@Business2024Secure" -c "Payment" --url "https://www.paypal.com" --notes "Business PayPal account" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add azure-storage -u azureuser -p "Azure@Storage2024Secure!" -c "Cloud" --url "https://portal.azure.com" --notes "Azure storage account" 2>nul
if %errorlevel% equ 0 (echo   ✓ azure-storage added) else (echo   ✗ Failed to add azure-storage)

REM GitLab
echo [12/15] Adding gitlab-work...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add gitlab-work -u gitlab.user -p "GitLab@Work2024Secure" -c "Version Control" --url "https://gitlab.com" --notes "Work GitLab instance" 2>nul
if %errorlevel% equ 0 (echo   ✓ gitlab-work added) else (echo   ✗ Failed to add gitlab-work)

REM MongoDB
echo [13/15] Adding mongodb-cluster...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add mongodb-cluster -u mongoadmin -p "MongoDB@Cluster2024!" -c "Databases" --url "mongodb://localhost:27017" --notes "MongoDB production cluster" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add "special-chars-test" -u "user@domain.com" -p "P@$$w0rd!#^&*()_+-=[]{}|;:',.<>?/" --url "https://example.com/test" --notes "Testing special characters in password" 2>nul
...
echo %TEST_PASSWORD% | %BINARY% --vault %TEST_VAULT% add random-service -u randomuser -p "RandomService@2024Secure" --url "https://random-service.example.com" --notes "Uncategorized test service" 2>nul
...
echo   - Version Control (4 credentials)
echo   - Uncategorized (2 credentials)
echo.
echo All credentials include full metadata:
echo   - Service, Username, Password (required)
echo   - Category, URL, Notes (optional, populated where applicable)
echo.
